...
def docker_buscar_container(container_nome):
...
def docker_parar_container(container_nome):
...
async def docker_criar_container(dockerfile_dir, container_nome):
...
def capturar_logs_com_erro_pelo_nome(container_name):
    try:
        client = docker.from_env()  # Crie um cliente Docker
        container = client.containers.get(container_name)  # Obtenha o objeto do container pelo nome
        logs = container.logs(stream=True, follow=True)  # Capture os logs e decode para texto
        linhas_de_erro = []
...
def monitorar_logs_em_tempo_real(container_name):
    try:
        client = docker.from_env()  # Crie um cliente Docker
        container = client.containers.get(container_name)  # Obtenha o objeto do container pelo nome
        ...
        for log in container.logs(stream=True, follow=True):
            log = log.decode('utf-8')  # Decodifique o log para texto
